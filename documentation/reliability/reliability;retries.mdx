...
Maximum delay is capped at **10 seconds** per attempt. Example with `X-Retry-Delay: 200ms` and `X-Retry-Count: 4`:
...
| `X-Proxy-Timeout` | `30s` | Per-attempt timeout. Accepts Go durations: `1s`, `5s`, `30s` |
