...
9. **Response** — The upstream response (headers + body) is returned to your app as-is. If the request was rescued by cache or failover, the header `X-Rescued: <reason>` is added.
...
|--------|--------|-------------|
| `X-Rescued` | `cache`, `failover`, `cascade_fallback` | Indicates how the request was saved. Only present when a rescue occurred. |
